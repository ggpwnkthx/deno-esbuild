...
Deno.test({
  name: 'cssPlugin with emitFile - emits CSS as separate file for CSS entry point',
  fn: async () => {
    const tmpDir = await Deno.makeTempDir()
    try {
      const entry = `${tmpDir}/entry.css`
...
Deno.test({
  name: 'cssPlugin with emitFile - injects CSS imported from nested non-entry point route',
  fn: async () => {
    const tmpDir = await Deno.makeTempDir()
    try {
      // Create entry point
