...
export function defs(makefile: string): Def[] {
  ...
  for (const t of parse(Deno.readTextFileSync(makefile))) {
    ...
    if (goos && goarch && bin) {
      ...
      throw new CliError(
        `Unable to classify platform target "${t.name}". Got GOOS=${goos ?? '<unset>'}, GOARCH=${
          goarch ?? '<unset>'
        }, BINPATH=${bin ?? '<unset>'}.`,
      )
    }
...
export function pick(all: readonly Def[], raw: string, wasm: boolean): Def[] {
  ...
  for (const x of want) {
    if (!known.has(x)) {
      throw new CliError(
        `Unknown platform or slug "${x}". Valid values: ${[...known].join(', ')}.`,
      )
    }
  }

  return all.filter((d) => d.kind === 'wasm' ? wasm && want.has('wasm') : want.has(d.slug))
}
