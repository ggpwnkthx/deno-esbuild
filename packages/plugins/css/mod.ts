...
export function cssPlugin(options: CssPluginOptions = {}): esbuild.Plugin {
  ...
  const entryPointOutputNames: Map<string, EntryPointOutputName> | null = emitFile
    ? new Map()
    : null

  // Track non-entry CSS imports (CSS imported from non-CSS entry points like TSX)
  const nonEntryCssImports: Map<string, TrackedCssEntry> | null = emitFile ? new Map() : null
  ...
  return {
    ...
    setup(ctx) {
      ...
      ctx.onStart(() => {
        ...
        const entryPointArray: Array<[string, string]> = Array.isArray(entryPoints)
          ? entryPoints.map((ep) => Array.isArray(ep) ? [ep[0], ep[1]] : [ep, ep])
          ...
          if (emitFile) {
            ...
            if (!isEntryPoint) {
              ...
              if (entryInfo) {
                ...
                const virtualName = `${entryInfo.outputName}_${entryInfo.virtualCssCounter}`
